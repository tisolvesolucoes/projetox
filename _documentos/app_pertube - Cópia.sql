...
CREATE TABLE IF NOT EXISTS `anuncios` (
  ...
  KEY `FK_anuncios_tipo_anuncios` (`TipoAnuncio`),
  CONSTRAINT `FK_anuncios_tipo_anuncios` FOREIGN KEY (`TipoAnuncio`) REFERENCES `tipo_anuncios` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
-- Copiando dados para a tabela app_pertube.anuncios: ~2 rows (aproximadamente)
...
INSERT INTO `anuncios` (`ID`, `IDUsuarios`, `TitiloAnuncio`, `TipoAnuncio`, `Estado`, `Cidade`, `Regiao`, `URLs`, `TextoAnuncio`, `DTCriacao`, `DTAtualizacao`) VALUES
	(43, 68, 'DSSDS', 4, 'SP', 'São Paulo', 'região sul', 'content/images/6598807329614/anuncio/rmg0uywKSNtE3IgzJCGU1KuX6iEX3UhZ.jpg', 'dsdsdsdsds', '2017-06-15 11:19:45', '2017-06-15 11:30:37'),
	(44, 68, 'gfdfgdgfdg', 5, 'BA', 'Aiquara', 'região norte', 'content/images/6598807329614/anuncio/DkKKvzFPFDiW9El5PVTSzIN1EhBXH4LO.jpg', 'gdfgdgdgdgdfg', '2017-06-15 11:31:29', '2017-06-15 11:30:37');
...
-- Copiando dados para a tabela app_pertube.auth: ~1 rows (aproximadamente)
/*!40000 ALTER TABLE `auth` DISABLE KEYS */;
INSERT INTO `auth` (`ID`, `IDUsuarios`, `IDSocket`, `IDUsername`, `DTLogin`, `Status`) VALUES
	(30, 68, 'SBEvm_PLzho0hg8iAAAA', '6532165647329', '2017-06-15 07:13:14', 1);
...
-- Copiando dados para a tabela app_pertube.curto: ~0 rows (aproximadamente)
...
-- Copiando dados para a tabela app_pertube.emails: ~1 rows (aproximadamente)
/*!40000 ALTER TABLE `emails` DISABLE KEYS */;
INSERT INTO `emails` (`ID`, `IDUsuarios`, `Emails`, `Status`) VALUES
	(141, 68, 'fabio.nsousa1@gmail.com', 1);
...
-- Copiando dados para a tabela app_pertube.enderecos: ~1 rows (aproximadamente)
...
INSERT INTO `enderecos` (`IDEndereco`, `CEP`, `Logradouro`, `Estado`, `Cidade`, `Regiao`, `Bairro`, `Numero`, `Complemento`, `Latitude`, `Longitude`) VALUES
	(92, '02303-130', 'Rua Doutor Natalino Righeto', 'SP', 'São Paulo', '', 'Tucuruvi', '768', '', '-23.4816539', '-46.61138769999999');
...
-- Copiando dados para a tabela app_pertube.info_usuarios: ~1 rows (aproximadamente)
...
-- Copiando dados para a tabela app_pertube.midia_social: ~1 rows (aproximadamente)
...
-- Copiando dados para a tabela app_pertube.telefones: ~1 rows (aproximadamente)
...
-- Copiando dados para a tabela app_pertube.toco: ~0 rows (aproximadamente)
...
INSERT INTO `usuarios` (`ID`, `IDEndereco`, `Nome`, `Sobrenome`, `Username`, `Senha`, `DTNascimento`, `SexoUser`, `DTCriacao`, `DTAtualizacao`, `Imagem`, `TipoAcesso`, `Status`) VALUES
	(68, 92, 'fabio2', 'sousa', '6532165647329', '753951', '1978-03-05', 1, '2017-05-27 17:54:32', '2017-05-27 17:54:32', 'content/images/6598807329614/thumbs/985c79ef744b470a9c7e3271e4ec17aajpg.jpg', 1, 1);
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `ANUNCIO`(IN `titulo` VARCHAR(50), IN `tipoAnuncio` INT, IN `estados` VARCHAR(50), IN `cidades` VARCHAR(50), IN `regiaoAnuncio` VARCHAR(50), IN `caminhoImagem` VARCHAR(250), IN `textoAnuncio` VARCHAR(50), IN `userLogado` INT)
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `ATUALIZA_FRM_GERAL`(IN `nome_user` VARCHAR(50), IN `sobrenome_user` VARCHAR(50), IN `username_user` VARCHAR(50), IN `senha_user` VARCHAR(50), IN `data_user` DATE, IN `sexo_user` INT, IN `sobre_user` VARCHAR(250)
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `EDITAR`(IN `userName` CHAR(50), IN `token` CHAR(50)
...
BEGIN
	...
	THEN
		INSERT INTO emails (IDUsuarios, Emails) VALUES (IDSockets, ' ');
...
BEGIN
	...
	THEN
		INSERT INTO telefones (IDUsuarios, Telefones) VALUES (IDSockets, ' ');
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `REMOVEANUNCIOS`(IN `IDAnuncio` INT, IN `IDUser` INT)
BEGIN
